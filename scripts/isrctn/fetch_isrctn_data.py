...
def fetch_and_store_trial(isrctn_id, cur):
    """
    Fetches and parses a single trial, executing all database inserts.
    Note: Transaction management (commit/rollback) is handled in the calling function.
    """
...
def main():
    ...
    try:
        ...
        for i, isrctn_id in enumerate(trials_to_fetch, 1):
            ...
            try:
                # 1. Fetch and store the trial data
                fetch_and_store_trial(isrctn_id, cur)
                
                # 2. Update the queue status to 'completed'
                cur.execute("""
                    UPDATE trial_queue 
                    SET retrieval_status = 'completed', 
                        last_attempt = CURRENT_TIMESTAMP,
                        error_log = NULL
                    WHERE isrctn_id = %s
                """, (isrctn_id,))
                
                # 3. Commit the entire unit of work
                conn.commit()
                success_count += 1
                
            except Exception as e:
                # Rollback any partial data changes
                conn.rollback()
                error_count += 1
                error_msg = str(e)
                ...
                # Record the failure in the queue (in a new transaction)
                try:
                    cur.execute("""
                        UPDATE trial_queue 
                    ...
                    print(f"Critical: Failed to log error for {isrctn_id}: {inner_e}")
                    conn.rollback()
